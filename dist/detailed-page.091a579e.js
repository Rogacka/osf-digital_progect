...
const visibleDetailedPageText = detailedPageText.innerText.slice(0, 100) + "...";
...
const imageOverlay = [
    ...detailedImageOverlay
];
const detailedPageContainer = document.querySelectorAll(".detailed-page__container");
...
imageOverlay.forEach((item, index)=>{
    item.addEventListener("click", ()=>{
        pageContainers.forEach((element)=>element.classList.remove("container-active"));
...
resizeButton.addEventListener("click", ()=>{
    pageImages.forEach((element)=>{
        if (!element.classList.contains("hide-img")) {
            ...
            biggestContainer.firstElementChild.style.backgroundImage = `url(${element.getAttribute("src")})`;
        }
    });
});
...
for (events of eventList)inputValue.addEventListener(events, function(event) {
...
//# sourceMappingURL=detailed-page.091a579e.js.map
